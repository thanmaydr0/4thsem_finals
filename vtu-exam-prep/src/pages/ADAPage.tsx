    ...
    async function fetchData() {
      try {
        ...
        const [modsRes, qsRes] = await Promise.all([
          supabase.from('modules').select('*').eq('subject_id', 'ada'),
          supabase.from('questions').select('*').eq('subject_id', 'ada').order('sort_order'),
        ]);
        ...
        console.error('Error fetching ADA data:', err);
